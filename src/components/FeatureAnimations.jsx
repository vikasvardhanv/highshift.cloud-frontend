...
export function VisualCalendarAnimation() {
 ...
 <span className="text-[10px] font-bold text-textMuted mb-1">{day}</span>
 <motion.div
 className={`w-full h-20 rounded-lg border-2 border-dashed transition-all ${draggedPost !== null && index === targetDay ? 'border-primary bg-primary/10' : 'border-borderColor bg-bgColor'}`}
 ...
 >
 <Calendar className="w-3 h-3 text-textMain" />
 </motion.div>
 ))}
 </motion.div>
 </div>
...
export function BestTimeAnimation() {
 ...
 />
 <span className="text-[9px] font-bold text-textMuted">{hour}:00</span>
 </div>
 );
 })}
 </div>
 ...
 exit={{ opacity: 0, scale: 0.5 }}
 className="absolute top-10 bg-emerald-500 text-textMain rounded-xl px-4 py-2 shadow-xl"
 >
...
export function MultiPlatformAnimation() {
 ...
 >
 <Layers className="w-10 h-10 text-textMain" />
 </motion.div>
...
export function AssetLibraryAnimation() {
 ...
 {[1, 2, 3, 4].map((i) => (
 <div key={i} className="border-2 border-dashed border-borderColor rounded-lg" />
 ))}
 </div>
 ...
 >
 <Image className="w-6 h-6 text-textMain" />
 </motion.div>
 ))}
...
export function ApprovalWorkflowAnimation() {
 ...
 {/* Step circle */}
 <motion.div
 className={`relative w-16 h-16 rounded-full flex flex-col items-center justify-center shadow-lg ${isComplete ? 'bg-emerald-500' : isActive ? 'bg-amber-500' : 'bg-white/10 bg-bgSurfaceHighlight'}`}
 animate={{
 ...
 >
 {isComplete ? (
 <CheckCircle className="w-8 h-8 text-textMain" />
 ) : (
 <Users className="w-8 h-8 text-textMain" />
 )}
 <span className="text-[8px] font-bold text-textMain mt-1">{step}</span>
 </motion.div>

 {/* Arrow connector */}
 {index < steps.length - 1 && (
 <motion.div
 className={`w-12 h-1 mx-2 ${isComplete ? 'bg-emerald-500' : 'bg-white/10 bg-bgSurfaceHighlight'}`}
 animate={{
 scaleX: isComplete ? [1, 1.1, 1] : 1
 }}
 transition={{ duration: 0.5 }}
 >
 <ArrowRight className={`w-4 h-4 -mt-2 ml-8 ${isComplete ? 'text-emerald-500' : 'text-textMuted'}`} />
 </motion.div>
 )}
 </div>
 );
...
export function CrossChannelAnimation() {
 ...
 <div key={i}>
 <motion.div
 className={`absolute ${p.bg} p-2.5 rounded-xl border border-borderColor`}
 animate={{
 x: x,
 y: y,
 opacity: [0.6, 1, 0.6]
...
export function CompetitorAnimation() {
 ...
 { label: 'Brand B', color: 'bg-white/20 bg-bgSurfaceHighlight' }
 ];
 ...
 <span className={`text-[10px] font-bold uppercase tracking-widest ${i === 0 ? 'text-primary' : 'text-textMuted'}`}>
 {comp.label}
 </span>
 <motion.span
 className="text-[10px] font-bold text-textMuted"
 ...
 <div className="h-2.5 w-full bg-white/10 bg-bgSurfaceHighlight rounded-full overflow-hidden">
 <motion.div
...
export function CustomDashboardAnimation() {
 ...
 key={id}
 layout
 className="glass-card bg-bgSurfaceHighlight border border-borderColor rounded-2xl flex flex-col items-center justify-center p-3 relative overflow-hidden group"
 transition={{ type:"spring", stiffness: 200, damping: 20 }}
 >
 ...
 <span className="text-[10px] font-bold text-textMuted uppercase tracking-tighter">{W.title}</span>
...
export function AutomatedReportsAnimation() {
 ...
 <div className="relative w-full h-48 flex items-center justify-center">
 {/* Clock context */}
 <div className="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-2 px-3 py-1 bg-bgSurfaceHighlight border border-borderColor rounded-full">
 <Clock className="w-3 h-3 text-textMuted" />
 <span className="text-[9px] font-bold text-textMuted uppercase">Every Monday @ 9AM</span>
 </div>

 {/* Path */}
 {/* Using a simple div since bg-dashed-gradient might not exist in utilities */}
 <div className="absolute w-40 h-[1px] border-t border-dashed border-borderColor" />
 ...
 <div className="bg-bgSurfaceHighlight p-3 rounded-2xl border border-borderColor">
 <Mail className="w-8 h-8 text-textMuted" />
 </div>
 ...
 <FileText className="w-4 h-4 text-textMain" />
 </div>
 </motion.div>
 </div>
 </div>
...
export function PostLevelInsightsAnimation() {
 ...
 <div className="relative w-full h-48 flex items-center justify-center">
 <motion.div
 className="relative z-10 w-32 bg-white dark:bg-surface border border-borderColor rounded-2xl overflow-hidden shadow-2xl"
 animate={{
 scale: zoomed ? 0.8 : 1,
 x: zoomed ? -30 : 0
 }}
 ...
 <div className="h-1.5 w-full bg-bgSurfaceHighlight rounded-full" />
 <div className="h-1.5 w-3/4 bg-bgSurfaceHighlight rounded-full" />
 </div>
 </motion.div>
 ...
 {/* Cursor */}
 <motion.div
 className="absolute z-30 text-textMain pointer-events-none"
 animate={{
...
export function SharableLinksAnimation() {
 ...
 return (
 <div className="relative w-full h-48 flex items-center justify-center border-t border-borderColor pt-4">
 {/* Generating visual */}
 <div className="relative w-56 h-12 bg-white dark:bg-black/40 border border-borderColor rounded-xl flex items-center px-4 overflow-hidden shadow-xl">
 <ExternalLink className="w-4 h-4 text-textMuted mr-3" />
 ...
 <motion.div
 className={`ml-2 px-3 py-1 rounded-lg text-[10px] font-bold transition-all ${status === 'copied' ? 'bg-emerald-500 text-textMain' : 'bg-primary/20 text-primary'}`}
...
export function UnifiedInboxAnimation() {
 ...
 <motion.div
 key={i}
 className="bg-bgSurfaceHighlight border border-borderColor p-2 rounded-xl flex items-center gap-3"
 initial={{ x: -100, opacity: 0 }}
 animate={{ x: 0, opacity: 1 }}
 transition={{ delay: m.delay, duration: 0.8, repeat: Infinity, repeatDelay: 2 }}
 >
 <div className="w-6 h-6 rounded-full bg-bgSurfaceHighlight flex items-center justify-center">
...
export function AutomatedRulesAnimation() {
 ...
 <motion.div
 key={tag}
 className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest border ${i === 0 ? 'bg-red-500/10 border-red-500/30 text-red-500' : 'bg-bgSurface/10 border-slate-500/30 text-textMuted'}`}
 animate={{ x: [0, 10, 0] }}
...
export function TeamCalibrationAnimation() {
 ...
 <div className="w-full max-w-[200px] bg-bgSurfaceHighlight border border-borderColor rounded-2xl p-4 relative shadow-2xl">
 ...
 >
 <span className="text-[8px] font-bold text-textMain">{user.label[0]}</span>
 {/* Typing indicator */}
 {i === 0 && (
 <motion.div
 className="absolute -bottom-1 -right-1 bg-bgSurface rounded-full p-1 shadow-lg"
 animate={{ scale: [1, 1.2, 1] }}
...
export function SavedRepliesAnimation() {
 ...
 <div className="w-32 bg-bgSurfaceHighlight border border-borderColor rounded-xl p-3 space-y-2">
 {['Hi there!', 'Thanks!', 'Refund...'].map((txt, i) => (
 <motion.div
 key={i}
 className="p-1 px-2 bg-bgSurfaceHighlight hover:bg-primary/20 rounded-md text-[8px] text-textMuted cursor-pointer overflow-hidden border border-transparent hover:border-primary/30 transition-all font-bold uppercase"
...
export function SentimentAnalysisAnimation() {
 ...
 <span className="text-[10px] font-bold text-textMuted uppercase">Analysis:</span>
...
export function SmartFilteringAnimation() {
 ...
 <Filter className="w-16 h-16 text-textMuted/20" />

 {/* Flowing items */}
 {[0, 1, 2, 3].map(i => (
 <motion.div
 key={i}
 className={`absolute w-3 h-3 rounded-full ${i % 2 === 0 ? 'bg-primary' : 'bg-bgColor'}`}
 animate={{
 x: [-60, 0, 60],
 y: [0, 0, i % 2 === 0 ? -40 : 40],
 opacity: [0, 1, 0]
 ...
 <div className="text-[8px] font-bold text-textMuted uppercase mb-1">Priority View</div>
 <motion.div
