    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center h-[calc(100vh-200px)]">
                <Loader2 className="animate-spin text-raven-500 w-8 h-8" />
            </div>
        );
    }
    ...
    return (
        <div className="h-[calc(100vh-140px)] flex gap-6 overflow-hidden max-w-[1600px] mx-auto">
            ...
            <div className="w-[380px] flex flex-col gap-6 shrink-0">
                ...
                {/* Add Creator Input */}
                <div className="bg-obsidian-950 border border-white/10 rounded-2xl p-5 shadow-lg">
                    ...
                    <div className="relative">
                        <input
                            ...
                            className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-gray-200 text-sm focus:outline-none focus:border-raven-500 focus:ring-1 focus:ring-raven-500/50 transition-all placeholder:text-gray-400"
                        />
                        <button className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-white/10 hover:bg-raven-600 rounded-lg text-gray-300 hover:text-white transition-colors">
                ...
                <div className="flex-1 bg-obsidian-950 border border-white/10 rounded-2xl p-5 shadow-lg flex flex-col overflow-hidden">
                    ...
                    <div className="flex items-center justify-between mb-4 bg-white/5/30 p-2 rounded-lg border border-white/10">
                        <div className="flex items-center gap-3">
                            <button className="text-xs font-medium text-gray-400 hover:text-white bg-white/5 hover:bg-white/10 px-3 py-1.5 rounded-md border border-white/10 transition-all">
                                Select All
                            </button>
                            <span className="text-[10px] text-gray-500">2 of 19 selected</span>
                        </div>
                        <button className="bg-raven-600 hover:bg-raven-500 text-white text-xs font-bold px-3 py-1.5 rounded-md shadow-lg shadow-raven-600/20 transition-all">
                            Get Reels
                        </button>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                        {accounts.map((account) => (
                            <motion.div
                                ...
                                whileTap={{ scale: 0.98 }}
                                className={`group p-3 rounded-xl border transition-all cursor-pointer relative overflow-hidden ${selectedCreator === account.accountId ? 'bg-raven-500/10 border-raven-500/50' : 'bg-white/5/30 border-white/10 hover:border-white/10' }`}
            ...
            <div className="flex-1 bg-obsidian-950 rounded-3xl overflow-hidden flex flex-col">
                <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 h-full">
                        {mediaItems.map((item) => (
                            <div key={item.id} className="relative group rounded-2xl overflow-hidden aspect-[9/16] bg-obsidian-950 border border-white/10 shadow-2xl">
                                {/* Media Background */}
                                <img
                                    src={item.thumbnail}
                                    alt={item.title}
                                ...
                                <div className="absolute top-4 left-4 z-20">
                                    <button className="p-2 bg-obsidian-950 backdrop-blur-md rounded-full text-white/70 hover:text-white hover:bg-raven-600 transition-all">
                                        <Bookmark className="w-4 h-4" />
                                    </button>
                                </div>
                                <div className="absolute top-4 right-4 z-20">
                                    <button className="p-2 bg-obsidian-950 backdrop-blur-md rounded-full text-white/70 hover:text-white transition-all">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center opacity-0 group-hover:opacity-100 transition-all transform translate-y-4 group-hover:translate-y-0 duration-300">
                                    ...
                                    <button className="flex items-center gap-2 px-5 py-2.5 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-white font-bold hover:bg-white hover:text-raven-900 transition-all">
                                ...
                                {/* Bottom Info */}
                                <div className="absolute bottom-0 left-0 w-full p-5 z-20 bg-obsidian-950/90 backdrop-blur-xl border-t border-white/5 transform translate-y-0 transition-transform">
                                    <div className="flex items-center justify-between mb-3">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[10px] text-gray-400">09/08/2026</p>
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex items-center justify-between text-[11px] font-medium text-gray-400 border-t border-white/5 pt-3">
                                        ...
                                        <button className="bg-raven-600 hover:bg-raven-500 text-white px-3 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide flex items-center gap-1.5 transition-colors">
