    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 p-8">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[3rem] p-12 shadow-2xl space-y-12">

                    {/* Main Form */}
                    <form onSubmit={handleSubmit} className="space-y-8">
                        <div className="grid md:grid-cols-2 gap-8">
                            <div className="space-y-4">
                                <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest pl-2">Core Topic</label>
                                <textarea
                                    required
                                    value={topic}
                                    onChange={(e) => setTopic(e.target.value)}
                                    placeholder="What's the story today?"
                                    className="w-full h-40 bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-3xl p-6 text-xl font-black italic uppercase italic tracking-tighter focus:outline-none focus:border-indigo-500 transition-all resize-none text-slate-800 dark:text-white placeholder:text-slate-400"
                                />
                            </div>
                            <div className="space-y-8">
                                <div className="space-y-4">
                                    <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest pl-2">Target Audience</label>
                                    <input
                                        type="text"
                                        required
                                        value={audience}
                                        ...
                                        className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl p-6 font-black italic uppercase italic tracking-tighter focus:outline-none focus:border-indigo-500 transition-all text-slate-800 dark:text-white placeholder:text-slate-400"
                                    />
                                </div>
                                <div className="space-y-4">
                                    <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest pl-2">Publish Date</label>
                                    <input
                                        type="date"
                                        required
                                        value={date}
                                        onChange={(e) => setDate(e.target.value)}
                                        className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl p-6 font-black italic uppercase italic tracking-tighter focus:outline-none focus:border-indigo-500 transition-all text-slate-800 dark:text-white"
                                    />
                                </div>
                            </div>
                        </div>
                                ...
                                {['instagram', 'facebook', 'linkedin', 'twitter'].map((p) => (
                                    <div key={p} className="space-y-3">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">{p}</span>
                                        </div>
                                        <div className="relative group">
                                            <input
                                                ...
                                                placeholder="@handle"
                                                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl px-5 py-4 text-sm font-bold text-slate-800 dark:text-white focus:outline-none focus:border-indigo-500 transition-all placeholder:text-slate-400"
                                            />
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-10">
                                <div className="space-y-2">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <h3 className="text-xl font-black italic uppercase tracking-tighter text-slate-900 dark:text-white">Engine Configuration</h3>
                                    </div>
                                    <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest pl-9">Distribution Strategy & Authentication</p>
                                </div>

                                <div className="flex-1 flex flex-col sm:flex-row items-center gap-6">
                                    {/* System Choice Icons */}
                                    <div className="flex items-center gap-3 p-2 bg-slate-100 dark:bg-slate-950 rounded-[2rem] border border-slate-200 dark:border-slate-800">
                                        <button
                                            type="button"
                                            onClick={() => {
                                            ...
                                            className={`p-4 rounded-full transition-all group relative ${apiKey ? 'hover:bg-white dark:hover:bg-slate-800 bg-white/50 dark:bg-slate-800/50' : 'bg-white dark:bg-slate-800 shadow-lg'}`}
                                            title="Social Raven"
                                        >
                                        ...
                                        <button
                                            type="button"
                                            className={`p-4 rounded-full transition-all group relative ${apiKey ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/40' : 'hover:bg-white dark:hover:bg-slate-800'}`}
                                    ...
                                    <div className="flex-1 w-full flex gap-3">
                                        <div className="flex-1 relative group">
                                            ...
                                            <input
                                                ...
                                                className="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl pl-16 pr-6 py-5 font-bold text-xs tracking-widest focus:outline-none focus:border-indigo-500 transition-all text-slate-800 dark:text-white placeholder:text-slate-400 shadow-sm"
                                            />
                                        </div>
                                        <button
                                            type="button"
