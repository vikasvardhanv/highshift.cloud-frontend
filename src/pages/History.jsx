...
export default function History() {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-24 gap-4">
            ...
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Retrieving Archive Logs...</p>
        </div>
    );
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                    <div className="relative group">
                        ...
                        <input
                            ...
                            className="bg-black/40 border border-white/10 rounded-2xl pl-12 pr-6 py-3.5 text-sm focus:outline-none focus:border-primary/50 focus:ring-4 focus:ring-primary/5 transition-all w-full sm:w-72 placeholder:text-gray-300"
                        />
                    </div>
            ...
            <div className="relative glass-card rounded-[2.5rem] overflow-hidden border-white/5 bg-white/[0.02] shadow-2xl">
                {filteredPosts.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-32 text-center px-6">
                        <div className="w-20 h-20 bg-obsidian-950 border border-white/5 rounded-3xl flex items-center justify-center mb-6 shadow-inner">
                            <FileX className="w-10 h-10 text-gray-300" />
                        </div>
                        <h3 className="text-xl font-extrabold mb-2">Null Set Detected</h3>
                        <p className="text-sm text-gray-500 max-w-xs font-medium">No transmission records match your current filter parameters.</p>
                    </div>
                ) : (
                    <>
                        <div className="overflow-x-auto scrollbar-hide">
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-white/[0.03] border-b border-white/5 text-[10px] text-gray-500 font-black uppercase tracking-[0.2em]">
                                    ...
                                    {paginatedPosts.map((post) => (
                                        <tr key={post.id || post._id} className="group hover:bg-white/[0.02] transition-all duration-300">
                                            <td className="p-8 max-w-sm">
                                                <div className="text-[13px] font-bold text-gray-200 line-clamp-2 leading-relaxed">
                                                    {post.content}
                                                </div>
                                            </td>
                                            <td className="p-8">
                                                <div className="flex -space-x-3">
                                                    {(post.target_accounts || []).slice(0, 4).map(acc => (
                                                        <div
                                                            key={acc.accountId}
                                                            className="w-10 h-10 rounded-xl bg-obsidian-950 border-2 border-obsidian-950 flex items-center justify-center text-[10px] font-black uppercase tracking-tighter text-gray-400 group-hover:border-white/10 transition-colors shadow-xl"
                                                            title={acc.platform}
                                                        >
                                            ...
                                            <td className="p-8 text-right">
                                                <button className="w-10 h-10 bg-obsidian-950 border border-white/5 rounded-xl flex items-center justify-center text-gray-500 hover:text-white hover:border-primary/50 transition-all opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0">
                        ...
                        <div className="p-8 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-6 bg-white/[0.01]">
                            <div className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">
                                Showing record <span className="text-gray-300">{((page - 1) * itemsPerPage) + 1}</span> to <span className="text-gray-300">{Math.min(page * itemsPerPage, filteredPosts.length)}</span> of <span className="text-gray-300">{filteredPosts.length}</span>
                            </div>
                            <div className="flex gap-3">
                                <button
                                    onClick={() => setPage(p => Math.max(1, p - 1))}
                                    disabled={page === 1}
                                    className="px-6 py-2.5 bg-obsidian-950 border border-white/5 hover:border-white/10 rounded-xl text-[10px] font-bold uppercase tracking-widest text-gray-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                                >
                                    Previous
                                </button>
                                <button
