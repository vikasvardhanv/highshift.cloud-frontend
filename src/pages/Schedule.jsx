...
export default function Schedule() {
 ...
 for (let i = 0; i < 7; i++) {
 days.push(
 <div className="col-span-1 text-center text-xs font-semibold text-textMuted uppercase py-2" key={i}>
 ...
 >
 <span className={`text-sm font-medium w-7 h-7 flex items-center justify-center rounded-full ${isSameDay(day, new Date()) ?"bg-primary text-textMain" :""}`}>
 {formattedDate}
 </span>
 ...
 {dayPosts.length > 2 && (
 <div className="mt-1 text-xs text-textMuted">+{dayPosts.length - 2} more</div>
 )}
 </div>
 );
 day = addDays(day, 1);
