...
export default function SmartInbox() {
 ...
 <span className="font-bold text-textMain truncate pr-2">{thread.sender_name}</span>
 <span className="text-xs text-textMuted shrink-0">
 {new Date(thread.updated_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
 </span>
 </div>
 <p className={`text-sm truncate ${!thread.is_read ? 'text-textMain font-medium' : 'text-textMuted'}`}>
 {thread.snippet}
 </p>
 </div>
 {!thread.is_read && (
...
function AccountRow({ account, checked, onToggle }) {
 ...
 <span className="block truncate text-sm font-extrabold text-textMain">{getAccountName(account)}</span>
 <span className="mt-1 block truncate text-xs font-medium text-textMuted">
...
function PlatformBadgeIcon({ platform }) {
 ...
 const Icon = meta?.icon || Inbox;
 return (
 <div className={`w-6 h-6 rounded-full flex items-center justify-center border-2 border-slate-900 ${meta?.bg || 'bg-gray-600'} text-textMain`}>
