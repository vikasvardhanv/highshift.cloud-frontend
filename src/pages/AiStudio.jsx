...
export default function AiStudio() {
    ...
    const tones = [
        ...
        { id: 'Minimalist', label: 'Minimal', icon: Bot, color: 'text-gray-400', bg: 'bg-obsidian-950/10 border-slate-500/20' }
    ];
    ...
    return (
        <div className="h-[calc(100vh-140px)] flex flex-col relative overflow-hidden rounded-3xl border border-white/10 bg-obsidian-950 shadow-2xl">
            ...
            {/* Header */}
            <div className="z-10 flex flex-col md:flex-row md:items-center justify-between p-6 border-b border-white/5 bg-obsidian-950 backdrop-blur-md">
                <div className="flex items-center gap-4 mb-4 md:mb-0">
                    <div className="relative">
                        ...
                        <div className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-obsidian-950 flex items-center justify-center">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-400">Multi-modal Creative Engine</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
                    {tones.map(t => (
                        <button
                            ...
                            ${tone === t.id
                                    ? `${t.bg} ${t.color} border-current shadow-lg shadow-raven-500/5 ring-1 ring-inset ring-white/5`
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-hide z-10">
                <AnimatePresence initial={false}>
                    {messages.map((msg, idx) => (
                        ...
                        >
                            {/* Avatar */}
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 shadow-lg border ${msg.role === 'assistant' ? 'bg-raven-500/10 border-raven-500/20 text-raven-400' : 'bg-white/5 border-white/10 text-gray-300'}`}>
                                {msg.role === 'assistant' ? <Sparkles className="w-4 h-4" /> : <User className="w-4 h-4" />}
                            </div>

                            {/* Message Bubble */}
                            <div className={`max-w-[80%] rounded-2xl p-5 text-sm leading-relaxed shadow-sm flex flex-col gap-2 ${msg.role === 'assistant' ? 'bg-obsidian-950/80 border border-white/10 text-gray-200 rounded-tl-sm' : 'bg-raven-600 text-white rounded-tr-sm shadow-raven-500/10'}`}>
                                ...
                                {msg.type === 'image' ? (
                                    <div className="rounded-xl overflow-hidden shadow-lg border border-white/10 mt-1">
                                        ...
                                        <div className="p-2 bg-obsidian-950 text-xs text-gray-400 flex justify-between">
                                ...
                                {msg.role === 'assistant' && !loading && (
                                    <div className="flex items-center gap-3 mt-2 pt-2 border-t border-white/5 opacity-50 hover:opacity-100 transition-opacity">
                                        {msg.model && <span className="text-[10px] uppercase font-bold text-gray-500">{msg.model}</span>}
                ...
                {loading && (
                    ...
                    >
                        ...
                        <div className="bg-obsidian-950 border border-white/5 px-4 py-3 rounded-2xl rounded-tl-sm flex items-center gap-1">
            ...
            {/* Input Area */}
            <div className="p-6 bg-obsidian-950 border-t border-white/5 z-20">
                <div className="relative max-w-4xl mx-auto">
                    ...
                    <input
                        ...
                        className="w-full bg-obsidian-950/80 border border-white/10 rounded-xl pl-5 pr-32 py-4 focus:outline-none focus:border-raven-500/50 focus:ring-1 focus:ring-raven-500/50 transition-all text-sm text-white placeholder:text-gray-500 shadow-xl"
                    />
                    <div className="absolute right-2 top-2 bottom-2">
                        <button
                            onClick={handleSend}
                            disabled={loading || !input.trim()}
                            className="h-full px-5 bg-raven-600 hover:bg-raven-500 disabled:opacity-50 disabled:hover:bg-raven-600 rounded-lg text-white text-xs font-bold uppercase tracking-wider transition-all transform hover:scale-[1.02] active:scale-[0.98] flex items-center gap-2"
                        >
